...
func _export_end() -> void:
	...
	# Command-line exports can exit before a deferred patch runs. Patch
	# immediately when the Xcode project is already available, and keep the
	# deferred path for editor exports that finish project creation later.
	if _patch_xcodeproj(export_dir):
...
func _patch_xcodeproj(export_dir: String) -> bool:
